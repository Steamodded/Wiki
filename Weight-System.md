...
- If your object isn't stored in `G.P_CENTERS`, you can add `SMODS.game_table_from_type[type_key]` that can be either a key in `G` (i.e. G["P_SEALS"]) or *(since 1814a)* a table of the form `{ ref_table = table, ref_value = key }`.
