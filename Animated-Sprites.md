...
- `frame_order: string`, `"linear"`, or `"random"`, or a table defining which frame appears at which index, e.g. `{[1] = 3, [2] = 1, [3] = 2}` (third frame -> first frame -> second frame). (defaults to `"linear"`) 

# StateSprite
For `StateSprite`s, most of the above holds, except its `sprite_args` contains a `states` field, which defines a table of states by key:
...
- `states`:
    - `[state_key 1]`: (the arbitrary name of the state, e.g. `moving`)
        ...
        - `frame_order: string`, `"linear"`, same as for `AnimatedSprite`, for this state.
