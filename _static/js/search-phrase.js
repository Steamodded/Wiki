...
// Credit: https://github.com/wfahren/Enhanced-Sphinx-Search
...
(function () {
  function initPhraseSearch() {
    ...
    Search.query = function (query) {
      ...
      if (query.includes("|")) {
        // Split by pipe and treat each part as individual words for OR search
        isOrSearch = true;
        console.log(query);
        phrases = query
          .split("|")
        ...
        if (hasQuotes) {
          ...
          // Unquoted search: split into phrases for AND search
          phrases = query
            .trim()
            .split(/\s+/)
            .filter((term) => term.length > 0)
  ...
  function highlightPhrases() {
    sessionStorage.setItem(
      "phrase_page_count",
      parseInt(sessionStorage.getItem("phrase_page_count") || "0") + 1,
    );
    ...
    if (query === '""' || query === "''") {
      ...
      const handledLocally = sessionStorage.getItem(
        "phrase_clear_handled_locally",
      );
      if (handledLocally) {
    ...
    const highlightKeys = allKeys.filter(
      (key) => key.includes("highlight") || key.includes("sphinx"),
    );
    ...
    const existingHighlights = document.querySelectorAll(
      'span.highlighted, .highlighted, span[style*="background-color"]',
    );
      ...
      highlightOperations.forEach((op) => {
        ...
        if (replacements.length > 0) {
          ...
          replacements.forEach((replacement) => {
            ...
            const phraseText = finalHTML.substring(
              replacement.start,
              replacement.end,
            );
            ...
            finalHTML =
              beforeText +
              '<span class="highlighted" style="background-color: yellow;">' +
              phraseText +
              "</span>" +
              afterText;
          });
  ...
  function interceptSearchForm() {
    ...
    const searchForms = document.querySelectorAll(
      'form[role="search"], #searchbox form, form[action*="search"], form:has(input[name="q"]), .wy-form form, form',
    );
    const searchInputs = document.querySelectorAll(
      'input[name="q"], input[type="search"], #searchbox input',
    );
    ...
    const allSearchForms = [
      ...new Set([...searchForms, ...formsWithSearchInputs]),
    ];
          ...
          parentForm.addEventListener(
            ...
            function (event) {
              ...
              if (query === '""' || query === "''") {
                ...
                const searchInput = parentForm.querySelector(
                  'input[name="q"], input[type="search"]',
                );
                if (searchInput) {
                  searchInput.value = "";
