...
- **Optional parameters** *(defaults)*:
  ...
  - `select_card`:
    ...
    - Set to a function `select_card(self, card, pack) -> string?, boolean?` to control if and where `card` should be saved for any `card, pack` combination. *(Added in 1814a)* Returning `true` as the second value allows the player to select whether to save or use the card.
